//! This library provides functionality to read a Matrix Market file, process it, and convert it into a compressed sparse matrix format.
...
/// Reads a chunk of a file asynchronously from a given start position to an end position.
...
/// Checks if the given `needle` sequence exists in the `haystack` sequence.
...
/// Loads a Matrix Market file asynchronously and converts it into a compressed sparse matrix.
pub async fn load_matrix_market(filename: &str) -> Result<CsMat<f64>, std::io::Error> {
